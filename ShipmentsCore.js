...
function resolveUaeWarehouseFromCourier_(courierRaw) {
  ...
  // Unknown couriers should not force a default warehouse; keep blank so downstream lookups can decide.
  if (!s) return '';
 ...
 * - Key = SKU + Warehouse (UAE) + Variant (uppercased)
...
function seedShipmentsUaeEgFromInventoryUae() {
  try {
    ...
    const seededTag = 'Seeded from Inventory_UAE';
    const makeSeedKey_ = function (sku, wh, variant) {
      const s = String(sku || '').trim();
      const w = normalizeWarehouseCode_(wh);
      const v = String(variant || '').trim().toUpperCase();
      if (!s || !w) return '';
      return s + '||' + w + '||' + v;
    };

    // Build inventory map: key = SKU||WH||Variant
    ...
    for (const r of invData) {
      ...
      const variantVal = invVarCol ? String(r[invVarCol - 1] || '').trim() : '';
      const key = makeSeedKey_(sku, wh, variantVal);
      if (!key) continue;
    ...
    // Index existing planning rows (Shipment ID blank): key = SKU||WH||Variant => first rowIndex
    ...
    for (let i = 0; i < shipData.length; i++) {
      ...
      const variantVal = shipVarCol ? String(row[shipVarCol - 1] || '').trim() : '';
      const key = makeSeedKey_(sku, wh, variantVal);
      if (!key) continue;

      if (!existingPlanRowByKey.has(key)) {
        existingPlanRowByKey.set(key, i);
      }
    }
    ...
    for (const [key, info] of invByKey.entries()) {
      ...
      if (shipNotesCol) {
        const note = `${seededTag} (OnHand: ${info.onHand})`;
        if (!String(row[shipNotesCol - 1] || '').includes(seededTag)) {
    ...
    for (const [key, info] of invByKey.entries()) {
      ...
      if (shipNotesCol) newRow[shipNotesCol - 1] = `${seededTag} (OnHand: ${info.onHand})`;
...
/**
 * Manual helper to dedupe seeded planned rows in Shipments_UAE_EG.
 * - Targets rows with blank Shipment ID and notes containing the seeded tag.
 * - Key = normalized Warehouse (UAE) + SKU + Variant (uppercased).
 * - Keeps the first occurrence and deletes later duplicates (safe to rerun).
 *
 * @returns {number} number of deleted rows
 */
function shipUaeEg_dedupeSeededPlannedRowsOnce_() {
  try {
    const sh = getSheet_(APP.SHEETS.SHIP_UAE_EG);
    const map = getHeaderMap_(sh);

    const colShipId = map[APP.COLS.SHIP_UAE_EG.SHIPMENT_ID];
    const colWh = map[APP.COLS.SHIP_UAE_EG.WAREHOUSE_UAE];
    const colSku = map[APP.COLS.SHIP_UAE_EG.SKU];
    const colVar = map[APP.COLS.SHIP_UAE_EG.VARIANT];
    const colNotes = map['Notes'];

    if (!colShipId || !colWh || !colSku || !colNotes) return 0;
    ...
    const makeKey_ = function (sku, wh, variant) {
      const s = String(sku || '').trim();
      const w = normalizeWarehouseCode_(wh);
      const v = String(variant || '').trim().toUpperCase();
      if (!s || !w) return '';
    ...
    for (let i = 0; i < data.length; i++) {
      const row = data[i];
      const shipId = String(row[colShipId - 1] || '').trim();
      if (shipId) continue; // not a planning row

      const notes = String(row[colNotes - 1] || '');
      if (notes.indexOf(seededTag) === -1) continue; // only auto-seeded rows

      const sku = String(row[colSku - 1] || '').trim();
      const wh = row[colWh - 1];
      const variantVal = colVar ? String(row[colVar - 1] || '').trim() : '';
      const key = makeKey_(sku, wh, variantVal);
      if (!key) continue;

      if (seen.has(key)) {
        rowsToDelete.push(i + 2); // +2 to convert to sheet row number
      } else {
        seen.add(key);
      }
    }
    ...
    logError_('shipUaeEg_dedupeSeededPlannedRowsOnce_', e);
    throw e;
  }
}
...
function _updateShipmentUaeEgRowTotalsAndStatus_(sh, rowIndex, headerMapOpt) {
  ...
  const colEta = map[APP.COLS.SHIP_UAE_EG.ETA] ||
    map['ETA'];
  const colActualArrival = map[APP.COLS.SHIP_UAE_EG.ARRIVAL] ||
    map['Actual Arrival'] ||
    map['Actual Arrival (EG)'];
  const colStatus = map[APP.COLS.SHIP_UAE_EG.STATUS] ||
    map['Status'];
  ...
  const arr = colActualArrival ? row[colActualArrival - 1] : '';
  ...
  try {
    ...
    try {
      ...
      if (typeof qc_recalcRows_ === 'function' && canRecalcMissingOk) {
        ...
        qc_recalcRows_(qcSh, qcMap, 2, rowsToRecalc);
      }
    } catch (e) { }
...
function syncQCtoInventory_UAE(opts) {
  ...
  try {
    ...
    function _normalizeUaeWarehouseOrBlank_(val) {
      ...
      // Only allow canonical UAE warehouses; unknowns are treated as blank to avoid silent misposts.
      if (norm === 'KOR' || norm === 'ATTIA') return norm;
      return '';
    }

    // Warehouse policy: prefer QC_UAE value (ATTIA/KOR). If blank, fallback to Purchases (same Line ID) when available; otherwise skip without defaults.
    ...
    if (purchLast >= 2) {
      ...
      const idxPurchLineId = purchMap[APP.COLS.PURCHASES.LINE_ID] ? purchMap[APP.COLS.PURCHASES.LINE_ID] - 1 : null;
      const idxPurchWarehouse = purchMap['Warehouse (UAE)'] ? purchMap['Warehouse (UAE)'] - 1 : (purchMap['Warehouse'] ? purchMap['Warehouse'] - 1 : null);
      ...
      purchData.forEach(function (r) {
        ...
        // Optional warehouse map for fallback when QC warehouse is blank.
        if (idxPurchLineId != null && idxPurchWarehouse != null) {
          const lid = String(r[idxPurchLineId] || '').trim();
          const wh = _normalizeUaeWarehouseOrBlank_(r[idxPurchWarehouse]);
          if (lid && wh && !purchWhByLineId[lid]) purchWhByLineId[lid] = wh;
        }
    ...
    qcData.forEach(function (row, idx) {
      ...
      if (!warehouse && qcLineId && purchWhByLineId[qcLineId]) {
